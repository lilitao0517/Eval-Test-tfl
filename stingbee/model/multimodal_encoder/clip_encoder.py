...
class CLIPVisionTower(nn.Module):
    def clip_interpolate_embeddings(self, image_size=600, patch_size= 14):
        ...
        if new_seq_length != seq_length:
            ...
            print("No interpolation needed; sequence length matches.")    
    def __init__(self, vision_tower, args, delay_load=False):
        ...
        if not delay_load:
            ...
            for layer in self.vision_tower.vision_model.encoder.layers[-4:]:  # Unfreeze the last 4 layers
                for param in layer.parameters():
                    param.requires_grad = True  

            # Also ensure the post_layernorm parameters are trainable if necessary
            self.vision_tower.vision_model.post_layernorm.weight.requires_grad = True
            self.vision_tower.vision_model.post_layernorm.bias.requires_grad = True        
    ...
    def load_model(self):
        ...
        for layer in self.vision_tower.vision_model.encoder.layers[-4:]:  # Unfreeze the last layers
            for param in layer.parameters():
                param.requires_grad = True  # Set requires_grad=True for the last 4 blocks
        # Also ensure the post_layernorm parameters are trainable if necessary
        self.vision_tower.vision_model.post_layernorm.weight.requires_grad = True
        self.vision_tower.vision_model.post_layernorm.bias.requires_grad = True   
