...
MODEL_PATH="/home/data2/zkj/llt_code/public_model/llava-1.5-7b-hf/"
for ((i=0; i<$NUM_GPUS; i++)); do
  CUDA_VISIBLE_DEVICES=$i \
  python stingbee/othermodel_eval/llava1_5_hf_classfication_eval.py \
    --model-path $MODEL_PATH \
    --question_file $QUESTION_FILE \
    --answers_file "$ANSWER_DIR/llava1_5_7B_pid_part_$i.jsonl" \
    --num_chunks $NUM_GPUS \
    --chunk_idx $i \
    --temperature 0.1 \
...
cat "$ANSWER_DIR"/llava1_5_7B_pid_part_*.jsonl | sort -k1 > "$ANSWER_DIR/llava1_5_7B_pid.jsonl"
