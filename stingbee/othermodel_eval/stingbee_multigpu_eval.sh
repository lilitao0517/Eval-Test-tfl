...
for ((i=0; i<$NUM_GPUS; i++)); do
  ...
  python stingbee/othermodel_eval/stingbee_classfication_eval.py \
    --model-path $MODEL_PATH \
    --question_file $QUESTION_FILE \
    --answers_file "$ANSWER_DIR/stingbee-7b_pid_part_$i.jsonl" \
    --num_chunks $NUM_GPUS \
    --chunk_idx $i \
    --batch_size 1 \
    --temperature 0.1 \
...
cat "$ANSWER_DIR"/stingbee-7b_pid_part_*.jsonl | sort -k1 > "$ANSWER_DIR/stingbee-7b_pid.jsonl"
